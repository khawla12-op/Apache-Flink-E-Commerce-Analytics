...
public class DataStreamJob {
	//The properties of jdbc connection
	private static final String jdbcUrl = "jdbc:postgresql://localhost:5432/postgres";
	private static final String username = "khaoula";
	private static final String password = "khaoula";

	public static void main(String[] args) throws Exception {
		// Sets up the execution environment, which is the main entry point
		// to building Flink applications.
		final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        //Set up a flink engine
		...
		KafkaSource<Transaction> source = KafkaSource.<Transaction>builder()
				...
				.setStartingOffsets(OffsetsInitializer.earliest())// reply the transactions from the start
				//Pour deserialiser les donnes comming from kafka
				.setValueOnlyDeserializer(new JSONValueDeserializationSchema())
				.build();
        //We should recognise that as a data stream
		DataStream<Transaction> transactionStream = env.fromSource(source, WatermarkStrategy.noWatermarks(), "Kafka source");

		transactionStream.print();
		//****************************************************************************************//

		//Our execution option
		JdbcExecutionOptions execOptions = new JdbcExecutionOptions.Builder()
				.withBatchSize(1000) //record
...
//		//create transactions table
		...
		//****************************************************************************************//
     //Working on inserting data into elasticsearch sink
		transactionStream.sinkTo(
				new Elasticsearch7SinkBuilder<Transaction>()
						.setHosts(new HttpHost("localhost", 9200, "http"))
						//Writing the transactions in the elastic search
						.setEmitter((transaction, runtimeContext, requestIndexer) -> {
                            //we need to convert the transactions comming from the steam to json before sending it to elastic search
